...
const ZONE_A_TIER = [{ key:'booth', name:'Booth', sqm:'8m²', count:9, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','2× LED arm light','— YOUNGS —','Std Counter','Furniture'] }];
const ZONE_B_TIER = [{ key:'booth', name:'Booth', sqm:'8m²', count:7, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','2× LED arm light','— YOUNGS —','Std Counter','Furniture'] }];
const ZONE_C_TIER = [
  { key:'m8', name:'Booth', sqm:'8m²', count:11, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','2× LED arm light','— YOUNGS —','Std Counter','Furniture'] },
  { key:'m14', name:'Booth', sqm:'14m²', count:1, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','4× LED arm light','— YOUNGS —','Std Counter','Furniture'] },
  { key:'m18', name:'Booth', sqm:'8+18m²', count:1, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','4× LED arm light','— YOUNGS —','Std Counter','Furniture'] },
];
const ZONE_D_TIER = [
  { key:'m8', name:'Booth', sqm:'8m²', count:8, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','2× LED arm light','— YOUNGS —','Std Counter','Furniture'] },
  { key:'m14', name:'Booth', sqm:'14m²', count:1, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','4× LED arm light','— YOUNGS —','Std Counter','Furniture'] },
  { key:'m82', name:'Booth', sqm:'8+2m²', count:1, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','2× LED arm light','— YOUNGS —','Std Counter','Furniture'] },
];
const ZONE_E_TIER = [{ key:'booth', name:'Booth', sqm:'8m²', count:8, req: ['Wooden backdrop (4×2.5mH)','42" TV + stand','Grey carpet','2× LED arm light','— YOUNGS —','Std Counter','Furniture'] }];
const ZONE_F_TIER = [
  { key:'sm', name:'Booth', sqm:'6m²', count:11, req: ['42" TV + stand','Grey carpet','2× LED arm light','— YOUNGS —','Std Counter','Pop Up','Displays and shelvings'] },
  { key:'md', name:'Booth', sqm:'8m²', count:8, req: ['42" TV + stand','Grey carpet','4× LED arm light','— YOUNGS —','Std Counter','Pop Up','Displays and shelvings'] },
  { key:'lg', name:'Booth', sqm:'14m²', count:2, req: ['42" TV + stand','Grey carpet','4× LED arm light','— YOUNGS —','Std Counter','Pop Up','Displays and shelvings'] },
];
...
ZONES: [
  ...
  { name: 'Keynote Hall', owner: 'Ari', status: 'Quoted', img: 'keynote-dark', renders: ['keynote-render', 'keynote-plan'], scope: 'AMG only provide stage carpet (6.1m × 21m ≈ 1,379 sq ft).', flag: '⚠ AMG scope: stage carpet only · 6.1m × 21m', blocking: true, req: ['Stage carpet (AMG scope)', 'All other items — by Youngs / Client'] },
  { name: 'AMA / Influencer Hub', owner: 'Ari', status: 'Quoted', img: 'ama-hub-0731', renders: ['ama-hub-0731'], drawings: ['ama-dwg-1'], scope: 'AMG shop drawing A.9 (VB, V0). Std panels + trainels (White Formica), 6× Custom LED Wall Frames, plywood stage skin (13 sheets) + white PVC cover, 4000×2500 LED wall (40 panels). Client: 4× arm chair (Youngs). Graphics: AMA-SHAPE-1/2/3 + AMA-LOGO vinyls · AVL: Johnathan', flag: 'Stage height — check LACC 30" rule',
    req: ['Std panels + trainels — White Formica (per A.9 inventory)', '6× Custom LED Wall Frame #01–06 — White Formica', '1× Stage Skin #12 — 6435×4155, 3/4" plywood re-skin (13 sheets)', '1× White PVC cover — 1980×4155', '1× LED Wall — 4000×2500mm, 40 LED panels', '4× Arm chair (from Youngs)', '1× Carpet — 6435×4155 (finish pendent)', 'Graphics: AMA-SHAPE-1/2/3 + AMA-LOGO vinyls', 'AVL (Johnathan)'] },
  { name: 'Match Meeting', owner: 'Ari', status: 'TBD', scope: 'TBD', flag: 'Furniture Youngs', req: ['Furniture (by Youngs)'] },
  { name: 'Breakout Session ×2', owner: 'Ari', status: 'Quoted', img: 'breakout-supplychain', renders: ['breakout-supplychain', 'breakout-ai'], drawings: ['breakout-dwg-1', 'breakout-dwg-2'], scope: 'AMG shop drawings A.10 (Session 1) + A.11 (Session 2), VB V0. Std panels + trainels, 4× custom LED wall frames, custom shape panel (S1 custom paint / S2 blue paint), curved + straight edge-lit LED stages, plywood stage skin, 3500×2000 LED wall (28 panels) each. Client: 2× arm chair + coffee table each. Graphics: BS-LOGO vinyl + BS-BK-WALL PVC · AVL: Johnathan', flag: 'Stage height — check LACC 30" rule',
  ...
  { name: 'Buyer Story', owner: 'Ari', status: 'Quoted', img: 'buyer-story-1', renders: [...gallery('buyer-story', 4)], scope: 'Youngs provide structure. AMG build + 1× 42" TV + stand.', flag: '⚠ Youngs structure · AMG build + TV',
    req: ['Structure (by Youngs)', 'AMG build', '1× 42" TV + stand'] },
  { name: 'Unboxing Live', owner: 'Ari', status: 'Quoted', img: 'unboxing-live-v3', renders: ['unboxing-live-updated'], drawings: [], scope: 'AMG shop drawing A.12 (Unboxing Live-R2, VB V0). 5× trainel (raw wood) + 2× custom angle panels, custom counter, 3× custom display boxes, 55" TV w/ stand. Client: camera + strobe + 2× ring-light tripod (Youngs). Graphics: BO fabric back walls (front+back), UL-CNTR + floor vinyl (5584×5584), RX-101/124 channels', flag: '⚠ Revised to R2 — now 55" TV + fabric back walls (was 42" TV + stand only)',
  ...
  { name: 'Supplier A200 — Zone A', owner: 'Jin & Chris', status: 'In Review', img: 'zone-a-map', renders: ['zone-a-map'], drawings: [], scope: 'A-01 ~ A-09. 9× 8sqm. Wooden backdrop (4×2.5mH) + 42" TV + stand + Std Counter + Grey carpet. A200 standard build.', flag: '9× 8sqm · 9 std counters',
  ...
  { name: 'Supplier A200 — Zone B', owner: 'Jin & Chris', status: 'In Review', img: 'zone-b-map', renders: ['zone-b-map'], drawings: [], scope: 'B-01 ~ B-08. 7× 8sqm. Wooden backdrop (4×2.5mH) + 42" TV + stand + Std Counter + Grey carpet. A200 standard build.', flag: '7× 8sqm · 7 std counters',
  ...
  { name: 'Supplier A200 — Zone C', owner: 'Jin & Chris', status: 'In Review', img: 'zone-c-map', renders: ['zone-c-map'], drawings: [], scope: 'C-01 ~ C-13. 13 booths. Wooden backdrop (4×2.5mH) + 42" TV + stand + Std Counter + Grey carpet. A200 standard build.', flag: '13 booths · 13 std counters',
  ...
  { name: 'Supplier A200 — Zone D', owner: 'Jin & Chris', status: 'In Review', img: 'zone-d-map', renders: ['zone-d-map'], drawings: [], scope: 'D-01 ~ D-10. 8/14/8+2sqm mixed. Wooden backdrop (4×2.5mH) + 42" TV + stand + Std Counter + Grey carpet. A200 standard build.', flag: '10 booths · 10 std counters',
  ...
  { name: 'Supplier A200 — Zone E', owner: 'Jin & Chris', status: 'In Review', img: 'zone-e-map', renders: ['zone-e-map'], drawings: [], scope: 'E-01 ~ E-08. 8× 8sqm. Wooden backdrop (4×2.5mH) + 42" TV + stand + Std Counter + Grey carpet. A200 standard build.', flag: '8× 8sqm · 8 std counters',
  ...
  { name: 'Podcast', owner: 'Iris', status: 'In Review', img: 'podcast', renders: ['podcast-2', 'podcast-3'], drawings: ['podcast-dwg-1'], scope: 'AMG shop drawing A.13 (VB, V0). Glass room — std panels (White Formica) + glass framing #01–09 + wooden beams, 12× tempered glass panels (3/8"). Client: 8× angle table, 2× on-air lightbox, 12× chair (Youngs). Graphics: PD-SHAPE-1/2/3/4 + PD-LOGO + 78× PD-SMALL-LOGO vinyls', flag: '⚠ Drawing has AMG building glass framing + glass — was "glass room by Youngs", confirm',
