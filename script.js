...
const IMG_CACHE_BUST = '?v=20260827e';  // bump to bust image CDN cache
...
const ZONE_A_TIER = [
  ...
  { key:'a02', name:'A-02', sqm:'8m²', count:1, renders: ['zone-a-render-9', 'zone-a-render-10'], req: ['Wooden backdrop (4×2.5mH)','42" TV + floor stand + Media Player','Std Counter — 990W×1000H×495D, Formica White','Grey carpet','2× LED arm light','4x) 700 (W)*18 (H)*300 (D), Shelf, Formica (WHT)','9x) 450 (W)*18 (H)*300 (D), Shelf, Formica (WHT)','26x) 10" Metal L-Bracket (WHT)','— YOUNGS —','Furniture'] },
...
ZONES: [
  ...
  { name: 'Supplier A200 — Block A', owner: 'Jin', status: 'Approved', img: 'zone-a-map-0825b', renders: ['zone-a-map-0825b', 'block-a-graphic-final-1', 'block-a-graphic-final-2'], drawings: [], scope: 'A-01 ~ A-09. 9× 8sqm. Wooden backdrop (4×2.5mH) + 42" TV + floor stand + Media Player + Std Counter + Grey carpet + AMG provide shelf & L-bracket.', flag: '9× 8sqm · 8 std counters · AMG shelf',
    req: ['8× Wooden backdrop (4×2.5mH)', '8× 42" TV + floor stand + Media Player', '8× Std Counter — 990W×1000H×495D, Formica White', '9× Grey carpet (≈792 sqft)', '16× LED arm light', '3× Metal Panel Footing', '— AMG Shelf —', '27× 700mm shelf, Formica (WHT) (23× @300D + 4× @400D A-08)', '4× 900mm shelf, Formica (WHT) (4× @400D A-02)', '3× 1500mm shelf, Formica (WHT)', '73× 450mm shelf, Formica (WHT)', '213× 10" Metal L-Bracket (WHT)', '— YOUNGS —', '8× Furniture', '2× Wire Grid 550×1100×50 (A-04 only)'], tiers: ZONE_A_TIER, units: makeTieredUnits('zone-a', ZONE_A_TIER, i => BOOTH_LABELS.a[i-1], n => [`zone-a-render-${n*2}`, `zone-a-render-${n*2-1}`]) },
  { name: 'Supplier A200 — Block B', owner: 'Jin', status: 'Approved', img: 'zone-b-map-0825', renders: ['zone-b-map-0825', 'block-b-graphic-final-1', 'block-b-graphic-final-2'], drawings: [], scope: 'B-01 ~ B-07. 5×8m² + 1×12m² + 1×16m². Wooden backdrop (4×2.5mH) + 42" TV + floor stand + Media Player + Std Counter + Grey carpet. A200 standard build.', flag: '7 booths · 7 std counters · 5×8 + 12 + 16m²',
    req: ['7× Wooden backdrop (4×2.5mH)', '7× 42" TV + floor stand + Media Player', '7× Std Counter — 990W×1000H×495D, Formica White', '7× Grey carpet (≈748 sqft)', '14× LED arm light', '4× Metal Panel Footing', '— YOUNGS —', '7× Furniture'], tiers: ZONE_B_TIER, units: makeTieredUnits('zone-b', ZONE_B_TIER, i => BOOTH_LABELS.b[i-1], n => [`zone-b-render-${n*2}`, `zone-b-render-${n*2-1}`]) },
