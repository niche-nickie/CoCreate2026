...
const BOOTH_LABELS = {
  ...
  c: i => `C-${String(i).padStart(2,'0')} (8-18m²)`,
...
ZONES: [
  ...
  { name: 'Keynote Hall', owner: 'Ari', status: 'TBD', img: 'keynote-dark', renders: ['keynote-render', 'keynote-plan'], scope: 'AMG only provide stage carpet (6.1m × 21m ≈ 1,379 sq ft).', flag: '⚠ AMG scope: stage carpet only · 6.1m × 21m', blocking: true, req: ['Stage carpet (AMG scope)', 'All other items — by Youngs / Client'] },
  { name: 'AMA / Influencer Hub', owner: 'Ari', status: 'In Review', img: 'ama-hub-0731', renders: ['ama-hub-0731'], drawings: ['ama-dwg-1'], scope: 'AMG shop drawing A.9 (VB, V0). Std panels + trainels (White Formica), 6× Custom LED Wall Frames, plywood stage skin (13 sheets) + white PVC cover, 4000×2500 LED wall (40 panels). Client: 4× arm chair (Youngs). Graphics: AMA-SHAPE-1/2/3 + AMA-LOGO vinyls · AVL: Johnathan', flag: 'Stage height — check LACC 30" rule',
  ...
  { name: 'Breakout Session ×2', owner: 'Ari', status: 'In Review', img: 'breakout-supplychain', renders: ['breakout-supplychain', 'breakout-ai'], drawings: ['breakout-dwg-1', 'breakout-dwg-2'], scope: 'AMG shop drawings A.10 (Session 1) + A.11 (Session 2), VB V0. Std panels + trainels, 4× custom LED wall frames, custom shape panel (S1 custom paint / S2 blue paint), curved + straight edge-lit LED stages, plywood stage skin, 3500×2000 LED wall (28 panels) each. Client: 2× arm chair + coffee table each. Graphics: BS-LOGO vinyl + BS-BK-WALL PVC · AVL: Johnathan', flag: 'Stage height — check LACC 30" rule',
  ...
  { name: 'Buyer Story', owner: 'Ari', status: 'In Review', img: 'buyer-story-1', renders: [...gallery('buyer-story', 4)], scope: 'Youngs provide structure. AMG build + 1× 42" TV + stand.', flag: '⚠ Youngs structure · AMG build + TV',
  ...
  { name: 'Supplier A200 — Zone A', owner: 'Jin & Chris', status: 'In Review', img: 'zone-a-render-1', renders: [...gallery('zone-a-render', 18)], drawings: [], scope: 'A-01 ~ A-09. 9× 8sqm. Wooden backdrop (4×2.5mH) + 42" TV + Std Counter + Grey carpet. A200 standard build.', flag: '9× 8sqm · 9 std counters',
    req: ['Wooden backdrop (4×2.5mH)', '42" TV', 'Std Counter', 'Grey carpet', 'Furniture (by Youngs)'], tiers: ZONE_A_TIER, units: makeTieredUnits('zone-a', ZONE_A_TIER, BOOTH_LABELS.a, n => [`zone-a-render-${n*2-1}`, `zone-a-render-${n*2}`]) },
  { name: 'Supplier A200 — Zone B', owner: 'Jin & Chris', status: 'In Review', img: 'zone-b-render-1', renders: [...gallery('zone-b-render', 14)], drawings: [], scope: 'B-01 ~ B-08. 7× 8sqm. Wooden backdrop (4×2.5mH) + 42" TV + Std Counter + Grey carpet. A200 standard build.', flag: '7× 8sqm · 7 std counters',
    req: ['Wooden backdrop (4×2.5mH)', '42" TV', 'Std Counter', 'Grey carpet', 'Furniture (by Youngs)'], tiers: ZONE_B_TIER, units: makeTieredUnits('zone-b', ZONE_B_TIER, BOOTH_LABELS.b, n => [`zone-b-render-${n*2-1}`, `zone-b-render-${n*2}`]) },
  { name: 'Supplier A200 — Zone C', owner: 'Jin & Chris', status: 'In Review', img: 'zone-c-render-1', renders: [...gallery('zone-c-render', 20)], drawings: [], scope: 'C-01 ~ C-11. 10× 8sqm + 1× 18sqm. Wooden backdrop (4×2.5mH) + 42" TV + Std Counter + Grey carpet. A200 standard build.', flag: '11 booths · 11 std counters',
    req: ['Wooden backdrop (4×2.5mH)', '42" TV', 'Std Counter', 'Grey carpet', 'Furniture (by Youngs)'], tiers: ZONE_C_TIER, units: makeTieredUnits('zone-c', ZONE_C_TIER, BOOTH_LABELS.c, n => [`zone-c-render-${n*2-1}`, `zone-c-render-${n*2}`]) },
  { name: 'Supplier A200 — Zone D', owner: 'Jin & Chris', status: 'In Review', img: 'zone-d-render-1', renders: [...gallery('zone-d-render', 20)], drawings: [], scope: 'D-01 ~ D-10. 8/14/8+2sqm mixed. Wooden backdrop (4×2.5mH) + 42" TV + Std Counter + Grey carpet. A200 standard build.', flag: '10 booths · 10 std counters',
    req: ['Wooden backdrop (4×2.5mH)', '42" TV', 'Std Counter', 'Grey carpet', 'Furniture (by Youngs)'], tiers: ZONE_D_TIER, units: makeTieredUnits('zone-d', ZONE_D_TIER, BOOTH_LABELS.d, n => [`zone-d-render-${n*2-1}`, `zone-d-render-${n*2}`]) },
  { name: 'Supplier A200 — Zone E', owner: 'Jin & Chris', status: 'In Review', img: 'zone-e-render-1', renders: [...gallery('zone-e-render', 16)], drawings: [], scope: 'E-01 ~ E-08. 8× 8sqm. Wooden backdrop (4×2.5mH) + 42" TV + Std Counter + Grey carpet. A200 standard build.', flag: '8× 8sqm · 8 std counters',
    req: ['Wooden backdrop (4×2.5mH)', '42" TV', 'Std Counter', 'Grey carpet', 'Furniture (by Youngs)'], tiers: ZONE_E_TIER, units: makeTieredUnits('zone-e', ZONE_E_TIER, BOOTH_LABELS.e, n => [`zone-e-render-${n*2-1}`, `zone-e-render-${n*2}`]) },
  { name: 'Supplier Non-A200 — Zone F', owner: 'Jin & Chris', status: 'In Review', img: 'zone-f-render-1', renders: [...gallery('zone-f-render', 39)], drawings: [], scope: 'F-01 ~ F-21. 4/6/8/14sqm mixed. 快幕秀 quick-pop backdrop + 42" TV + Fabric display + shelving (Youngs).', flag: '21 booths · 21 std counters',
    req: ['快幕秀 quick-pop backdrop', '42" TV', 'Fabric display + shelving (by Youngs)'], tiers: ZONE_F_TIER, units: makeTieredUnits('zone-f', ZONE_F_TIER, BOOTH_LABELS.f, n => [`zone-f-render-${n*2-1}`, `zone-f-render-${n*2}`]) },
